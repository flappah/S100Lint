    ...
    public class FeatureCatalogueParser : S100LintBase, IFeatureCatalogueParser
    {
        ...
        private List<IReportItem> SimpleTypeChecker(XmlNodeList nodes, XmlNamespaceManager fcNsManager, XmlDocument[] schemaDocuments, XmlNamespaceManager schemaNsManager)
        {
            ...
            foreach (XmlNode catalogueNode in nodes)
            {
                ...
                XmlNode fcValueTypeNode = catalogueNode.SelectSingleNode(@"S100FC:valueType", fcNsManager);

                if (fcNameNode != null && fcValueTypeNode != null)
                {
                    // first check if valuetype is enumeration or list
                    if (fcValueTypeNode.InnerText.ToUpperInvariant().In("LIST", "ENUMERATION"))
                    {
                        // if so check if the simpletype has been defined in the xsd
